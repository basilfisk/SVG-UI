...
(function() {
	var vs = {
		...
		//--------------------------------------------------------------------------------------
		ajaxSynchJSON: function(file) {
			var request;
			
			// Initiate the request for data from the server
			request = new XMLHttpRequest();
			request.open("GET",file,false);			// DEPRACATED: http://xhr.spec.whatwg.org/
//			request.open("GET",file,true);
// TODO Sort out synchronous loading
			request.setRequestHeader('Content-Type','application/json');
			request.setRequestHeader('Content-Length',0);
			request.send('');
	
			// Return the JSON object, or undefined if an error occurs
			switch(request.getResponseHeader('Content-Type')) {
				// JSON response
				case 'application/json':
					try {
						// If the JSON file is read, return the document object
						if(request.responseText) {
							return JSON.parse(request.responseText);
						}
						// No root element in the JSON document.  Return undefined
						else { vs.message("No data in JSON object"); return; }
					}
					// Problem handling the XML document.  Return undefined
					catch(e) {
						vs.message("Cannot parse JSON: " + request.responseText);
						return;
					}
				// Non XML response.  Return undefined
				default:
					vs.message("Unrecognized response header: " + request.responseText);
					return;
			}
		},
...
*/
		...
		// --------------------------------------------------------------------------------------
		that.addSymbol = function(defs,id) {
			var symbol,shape;
...
// TODO Lists must be set up, if not, issue warning
		...
		// --------------------------------------------------------------------------------------
		that.styles = function(args) {
			var style = {
				styleObj: {}
			};
			
			// --------------------------------------------------------------------------------------
			// Load style data from the server into a style object
			//
			// Return the fully populated style object
			// --------------------------------------------------------------------------------------
			style.load = function() {
				// Load the style definitions from the web site
				var styles = vs.ajaxSynchJSON(that.stylesheet);
				
				// Read all child nodes
				style.treewalk(styles);
			};
			...
			// Load the style data if object is empty
			if(Object.keys(style.styleObj).length == 0) {
				style.load();
			}
		...
		//	- Start by checking the browser capabilities
		...
		// Read the application definition file from the server and store in a JSON object
		var elements = vs.ajaxSynchJSON(that.definitions);
		
		// Create the form definitions
		vs.defnForms = that.formDefinitions(elements);
		
		// Create the grid definitions
		vs.defnGrids = that.gridDefinitions(elements);
